...
true_photo_dir=/var/www/latroy/photos/photo_files/
true_notes_dir=/var/www/latroy/notes/note_files/
true_audio_dir=/var/www/latroy/music/audio_files/
true_lyrics_dir=/var/www/latroy/music/lyrics_files/
...
generate_differences() {
  for filepath in "$1"*
  do
    echo ./$(basename $1)/$(basename $filepath) >> /var/www/latroy/dir_files.compare
  done
    ...
    then
      case $1 in
        ...
        "songs")
           ...
           echo -n "Lyrics file: "
           read lyrics </dev/tty
           echo "INSERT INTO songs ($song_columns) VALUES ('$title', '$blurb', '$date', '$change', './lyrics_files/$lyrics');" >> $sql_output 
           ;;
      esac
    elif [ $3 == "REMOVE" ]
    then
